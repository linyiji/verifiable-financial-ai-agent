...
生成能力构建使用独立、明确的 `teamorouter-terra` 路由，请求和实际模型必须同为 `gpt-5.6-terra`。不把 Sol/Luna 的返回重命名为 Terra，也不预先认证能力。规格编译、沙箱验证和注册批准仍是必需门槛。模型身份不符时停止生成，不重试。已耗尽的明确传输／结构化输出生成故障仅影响对应输出；认证、授权、持久化和未知契约错误仍会终止流程。来源和依赖进度事件遵循冻结的事件格式，并使用任务真实记录的进度。
