...
| 公开图片 | 获准来源 | 状态 |
| --- | --- | --- |
| autonomous-research.png | S03-autonomous-research.png | PASS；历史已发布执行 |
| financial-review.png | S07-financial-review.png | PASS；54 项真实检查，展开了 Growth 恒等式检查 |
...
真实路由基础路径：/runs/RUN-ab806291-8cbf-4c9b-8852-b6a54f10768d?stage=research；该 Run 下的 /results/review 和 /results/execution；/objects/OBJ-NVDA?tab=memories 和 ?tab=compare。

完整截图验收门比较了浏览前／后，以及截图操作后的 48 张生产表的计数／哈希：零变化，包括投影缓存。零次新建 Run／供应商／模型调用。这不意味着克隆仓库时会附带 Owner 的历史数据。

报告来源／贡献覆盖仍为 PARTIAL。恢复画面的 Execution 视图可能显示其 PARTIAL 能力标签；但对于捕获到的这一次有界事件，展示的恢复证据本身是完整的。完整 Claim Trace 抽屉与延期 PDF 功能仍不可用。已观测指标／主张／问题以外的 Memory 类别仍为 NOT_OBSERVED。
