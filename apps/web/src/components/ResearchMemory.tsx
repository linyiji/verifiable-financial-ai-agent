...
export function ResearchMemory({memory, unavailable, onOpenSource, pane = "all"}: {
  readonly pane?: "all" | "current" | "memories" | "compare";
  ...
  const filter = ({metrics:"VERIFIED_METRIC",claims:"VERIFIED_CLAIM",issues:"RESOLVED_ISSUE"} as Record<string,string>)[comparison];
  ...
  return <>
    {(pane === "all" || pane === "current") && <section className="card pad memory-current" data-testid="current-research-view" data-source-run-id={view.source_run_id} data-view-version={view.research_view_version_id} data-object-version={memory.object_version?.object_version_id}>
      ...
      {pane === "current" && view.items.filter(i=>i.category !== "RESOLVED_ISSUE").map(i=><article className="memory-item" key={i.memory_item_id}><div><strong>{i.title}</strong><p>{i.statement}</p></div><button className="btn ghost sm" onClick={()=>onOpenSource(i.source_run_id,i.report_anchor)}>查看精确来源 →</button></article>)}
    </section>}
    {(pane === "all" || pane === "memories") && <section className="card pad research-memory" data-testid="research-memory">
    ...
    {(pane === "all" || pane === "compare") && !memory.base_memory?.current_view && <section className="card pad"><h2>对比</h2><p>尚无足够的受治理历史视图可供比较。</p></section>}
    {(pane === "all" || pane === "compare") && memory.base_memory?.current_view && memory.incremental_context && <section className="card pad" data-testid="base-vs-current" data-base-run-id={memory.base_memory.current_view.source_run_id} data-current-run-id={view.source_run_id}>
      ...
      <div className="comparison-tabs" role="group" aria-label="比较类别">{[["overview","概览"],["metrics","Metrics"],["claims","Claims"],["issues","Issues"],["process","Research Process"]].map(([key,label])=><button className={`btn ${comparison===key ? "primary" : "ghost"} sm`} aria-pressed={comparison===key} key={key} onClick={()=>setComparison(key)}>{label}</button>)}</div>
      {comparison==="process" && <p>尚无足够的受治理研究过程对比数据；以下计划背景不代表任务或耗时节省。</p>}
      <div data-testid="comparison-changes">{memory.changes?.filter(c=>comparison!=="process" && (!filter || c.category===filter)).map((c,index)=><p key={index} data-testid="governed-change" data-change={c.change} data-category={c.category}>{names[c.category]}：{({UNCHANGED:"数值未变 · 本次独立验证",UPDATED:"已更新",REVALIDATED:"已重新验证",NEW:"本次新增",REMOVED_FROM_CURRENT_VIEW:"仅保留在历史视图"})[c.change]}</p>)}</div>
      <div className="object-comparison-columns">{[memory.base_memory.current_view,view].map((v,index)=><div key={v.research_view_version_id}>
        <h3>{index===0 ? "Base · 上次研究" : "Current · 当前研究"} · View v{v.research_view_version}</h3><code className="comparison-run-id">{v.source_run_id}</code><p>{v.summary ?? "未观察到已验证摘要"}</p>
        ...
        {v.items.filter(i=>comparison!=="process" && (!filter || i.category===filter)).map(i=><article className="memory-item" key={i.memory_item_id} data-testid="comparison-item" data-source-run-id={i.source_run_id}>
