...
class Session:
    ...
    async def scalars(self, statement):
        ...
        return SimpleNamespace(all=lambda: [SimpleNamespace(payload=SimpleNamespace(payload={"status":"FAILED"}))])
...
def setup(monkeypatch):
    for name in ("ResearchObject", "ResearchRun", "ResearchGoal", "ResearchSchemeSnapshot",
                 "PlannedTaskGraph", "ActualRuntimeGraph", "CompletedRunArtifacts", "RuntimeEvent"):
    ...
    def build(**kwargs):
        run=kwargs["run"]
        return Projection({"run_id":run.run_id,"status":run.status.value,
    ...
    def make(status, cache):
        ...
        owner.row=SimpleNamespace(object_id="OBJ-A",projection_revision=12,projection_sequence=17,
           payload={"run":SimpleNamespace(run_id="RUN-A",status=status),"goal":"goal-authority",
                    "scheme":"scheme-authority","runtime":{"planned_graph":"planned-authority",
                    "actual_graph":"actual-authority"},"artifacts":"artifacts-authority"})
...
@pytest.mark.parametrize("cache",[None,{"revision":0,"payload":"stale"}, {"revision":12,"payload":"existing"}])
async def test_repeated_and_concurrent_reads_never_write(setup,status,cache):
    ...
    assert first.facts=={"run_id":"RUN-A","status":status.value,"scheme":"scheme-authority","revision":12,"sequence":17}
