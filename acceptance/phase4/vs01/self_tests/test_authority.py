...
def test_frozen_contract_authority_is_revision_qualified() -> None:
    ...
    assert len(result["contract_components_verified"]) == 14
    ...
    assert result["revision_bound_addenda_verified"] == ["final_freeze_decision"]
...
def test_authority_rejects_a_tampered_revision_bound_addendum(tmp_path: Path) -> None:
    authority = json.loads((VS01_ROOT / "contract_authority.json").read_text(encoding="utf-8"))
    authority["revision_bound_addenda"][0]["sha256"] = "0" * 64
    tampered = tmp_path / "authority.json"
    tampered.write_text(json.dumps(authority), encoding="utf-8")
    ...
    try:
        ...
        assert "final_freeze_decision bytes hash mismatch" in str(error)
    else:
        raise AssertionError("tampered revision-bound addendum must fail closed")
